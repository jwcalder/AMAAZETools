...
# -*- coding: utf-8 -*-
"""
Created on Fri Jan 29 00:31:45 2021
@author: rileywilde
"""
...
def svi(P,T,r,ID):
  ...
  #Bool indicating at which vertices to compute SVI 
    ...
    for i in np.arange(0,rlen): 
        svipca_module.svi(P,T,ID,r[i],eps,prog,S,G)
        Sout[:,i] = S
        Gout[:,i] = G
    return Sout,Gout

#the next three functions are copied from the GitHub for normals for svipca:
...
def svipca(P,T,r,ID):
    ...
    for k in np.arange(0,rlen):
        ...
        visnegative = S != -1;
        vvneg = np.matlib.repmat(np.double(visnegative[:,None]),1,3)
        K1[visnegative,k] = 0; 
        K2[visnegative,k] = 0;
        V1[:,l] = vvneg*V1[:,l]    
        V2[:,l] = vvneg*V2[:,l]
        ...
        vecneg = np.matlib.repmat(vecneg[:,None],1,3)
        V3[:,l] = vecneg*V3[:,l];
        V2[:,l] = vecneg*V2[:,l];
        V1[:,l] = vecneg*V1[:,l];
